...
export default function EditBrand() {
    ...
    useEffect(() => {
        client.get(`brand/${id}`).then(
            (res) => {
                ...
                const selected = data.categoryId.map((cat) => ({
                    value: cat._id,
                    label: cat.name
                }))
    ...
    const fetchData = (e) => {
        ...
        payload.append("name", e.target.brand.value),
            ...
            payload.append("categories", JSON.stringify(selCategory.map((cat) => cat.value))),
            payload.append("image", e.target.image.files[0])

        client.put(`brand/update/${id}`, payload).then(
            (res) => {
                if (res.data.success) {
                    e.target.reset()
                }
                router.push("/admin/brand")
                notify("Brand Edited", res.data.success)
            }
        ).catch(
            (error) => {
                console.log(error)
            ...
            <div className="w-full max-w-lg bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Update brand details and categories
                    </p>
                </div>
                    ...
                    <div className="mt-2 flex items-center gap-3">
                        {image && (
                            <img
                                src={image}
                                alt="preview"
                                className="w-14 h-14 object-cover rounded-lg border border-gray-200 bg-white p-1"
                            />
                        )}
